...
1.  **What is the author defining in lines 15-17? Where else in the code do these defined variables show up? What advantage(s) is there to defining these values here, as variables, rather than defining the values directly throughout the script?**
    ...
    -   The author is defining alien as black and green, bg as black, and accent as green. These show up later on in the code for text and backgrounds in the ggplots. The advantage to defining them separate is you can easily change the colors by editing `alien <- c("#101319", "#28ee85")`
...
2.  **In your own words, explain what the function, `font_add_google()`, does. What's the difference between the two arguments, `name` and `family`?**
    ...
    -   The `font_add_google()` is from the sysfonts package and it allows you to use fonts from the Google fonts library. The name argument is for the exact name of the Google Font you want to use and the family argument is the label you give to the font to reference it within R. 
...
3.  **Describe what this data frame contains.**
    ...
    -   This data frame contains the summed population of each state in the US.
...
4.  **Describe what this data frame contains.**
    ...
    -   This data frame contains the number of UFO sighting observations, total population, UFO observations per 10,000 people, and opacity value for each state.

5.  **What does `opacity_val` represent, and why is it calculated?**

    -   `opacity_val` represents how many UFO sightings each state has compared to the state with the most UFO sightings per 10,000 people. This is calculated to make visualizations where we can use color to show differences in intensity.
...
6.  **Describe what this data frame contains.**
    ...
    -   This data frame contains the total sightings, and opacity value for each of the top 10 UFO shapes.
...
7.  **What does `fct_reorder` do when it is applied to the `shape` variable? What would have happened if this step was not performed?**
    ...
    -   When `fct_reorder` is applied to the shape variable it reorders shape column by total_sightings, so that shapes with more total_sightings are at the top and shapes with fewer total_sightings are on the bottom. If this step was skipped the shapes would stay in their original order.

8.  **What is the purpose of rescaling `opacity_val`? And why rescale from 0.3 to 1?**

    -   The point of rescaling `opacity_val` was to make each shape more visible when visualizing. If the `opacity_val` were to be 0, the shape wouldn't be visible when making a visualization.
...
9.  **Describe what this data frame contains.**
    ...
    -   This data frame contains the amount of UFO sightings for the 7 days of the week and each hour of the day, opacity value, and hour labels.

10. **What is the purpose of the last line inside the `case_when()` statement (`TRUE ~ paste0(hour - 12, "pm")`)?**

    -   The point of the statement (`TRUE ~ paste0(hour - 12, "pm")`) is to subtract all remaining hours by 12 and add "pm" to them. 
...
11. **In your own words, what is the difference between `paste0()` and `glue()`? Why did the author use `paste0` to construct `quotes` and `glue` to construct the other text elements?**
    ...
    -   `paste0()` just joins strings together, and `glue()` lets you insert variables into a string using {} and use HTML formatting. The author used `paste0` to construct `quotes` because `quotes` is just simple text, while the other text elements included more complex variables and HTML formatting
...
12. **Explain the values provided to the `x` aesthetic for both text geoms (`shape` & `total_sightings`).**
    ...
    -   The `x` aesthetic for both text geoms (`shape` & `total_sightings`) determines where the text labels will show up on the horizontal axis. x = 200 puts the shape labels at the start of the bar, and x = total_sightings-200 puts the text at the end of the bar.
...
13. **Consider the order of `geom_*()` layers in the the above plot (`plot_us`). Why did the author order the layers in this way?**
    ...
    -   The author put the `geom_*()` layers in this order so the colored background made by geom_rect() ends up at the back, allowing you to see the text in front of it.
...
14. **This plot includes one-letter labels for each day of the week. How is ths accomplished when week days are written using their three-letter abbreviations (e.g. `Mon`, `Tue`) in the `df_day_hour` data frame?**
    ...
    -   This is accomplished with `str_sub(wday, start = 1, end = 1)`. This line of code only uses the first letter of each character string in the wday column.
...
15. **What role do the `ylim()` and `xlim()` functions play in shaping a ggplot, and how do they change the visual layout of this particular plot? To better understand their effect, try rerunning the code with each of these lines commented out and observe how the plot’s spacing and composition change.**
    ...
    -   When shaping a ggplot `ylim()` and `xlim()` functions set the limits of the y-axis and x-axis. In `plot_day`, the `ylim()` and `xlim()` functions set the limits of the space around the tiles and text.
...
16. **Why do you think the author chose to convert these text elements (and also in `plot_ufo`, below!) into ggplot objects (you may consider returning to this question after you've worked your way through all of the code)?**
    ...
    -   They were made into ggplot objects to combine in a bigger plot made later on. These are just quotes and don't really contribute much on their own.
...
17. **Why does the author render `plot.caption` using `ggtext::element_markdown()`, rather than `element_text()` (like he does for rendering `plot.title` and `text`)?**
    ...
    -   The author renders `plot.caption` using `ggtext::element_markdown()`, rather than `element_text()` because it contains HTML formatting and markdown elements.
...
18. **Explain how `plot_final` is assembled. What do you think is the most challenging aspect of arranging all components into a single plot?**
    ...
    -   Its assembled by using plot_base as the base and insetting all other plots and text elements using `inset_element()`. I think the hardest part as figuring out the sizing and positioning for all of the parts of this plot.

19. **Can you think of one reason the author may have chosen to separate the construction of `plot_base` and `plot_final`?**

    -   To make it easier to edit individual parts later on. 
...
20. **During week 2, we discuss [Choosing the right graphic form](https://eds-240-data-viz.github.io/course-materials/lecture-slides/lecture2.1-choosing-graphic-forms-slides.html#/title-slide). Refer to this lecture when answering the sub-questions, below:**
    ...
    a.  **What "perceptual tasks" (from Cleveland & McGill's heirarchy) must the viewer perform to extract information from these visualizations?**
        ...
        -   Compare lengths, shadding and saturation, and angles
    ...
    b.  **What task(s) do you think the author wanted to enable or message(s) he wanted to convey with these visualizations (see lecture 2.1, slide 16 for examples)? Be sure to note at least one task / message for each of the three data viz.**
        ...
        -   spatial patterns (US map sightings per 10K population) - the author wanted to show which states have the most sightings per 10K population with a map of the US.
        -   temporal patterns (Donut plot of sightings at different times of the day) - the author wanted to show which times UFOs are most frequently observed.
    ...
    c.  **Name at least one caveat to the "hierarchy of perceptual tasks" that the author employed to achieve a goal(s) you noted in question b?**
        ...
        -   A caveat was that the author made the US map with square states instead of their actual shape. This made it hard to tell which was which. Also, some of the states were really dark, which made it hard to see the state label.

21. **Describe two elements of this piece that you find visually-pleasing / easy to understand / intuitive. Why?**

    -   I really like the UFO in the middle of the donut chart and how the donut chart uses brightness to show what time the most sightings took place. This plot is super intuitive. You can just look at it and know exactly what is going on.

22. **Describe two elements of this piece that you feel could be better presented in a different way. Why?**

    -   I think the sightings per 10K population should have been displayed as a map of the United States instead of squares; it would make it more clear which state is which. Also, for the bar graph on the top, I don't like how the bottom bars are so dark; you can barely read the words on it. I think all the bars should be the same color.
...
23. **Describe two new things that you learned by interpreting / annotating this code. These could be packages, functions, or even code organizational approaches that you hadn't previously known about or considered.**
    ...
    -   I learned about the `glue()` function. I really like that it allows me to add HTML elements.
    -   I thought `inset_element()` was pretty cool. I never knew you could store ggplot elements and add them together.
...
24. **How, if at all, did you use AI tools to help you interpret this code? Describe your approach to using these tools for this assignment. In what ways was consulting the documentation more (or less) helpful than using AI?**
    ...
    -   I used AI to help better understand arguments for this code because sometimes the documentation wording is just too confusing.
